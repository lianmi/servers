...
func CreateInitControllersFn(
	...
	return func(r *gin.Engine) {
		...
		storeGroup.Use(authMiddleware.MiddlewareFunc())
		{
			...
			storeGroup.GET("/getallopks", pc.GetAllOPKS)                 //获取当前商户的所有OPK
			storeGroup.POST("/eraseopk", pc.EraseOPK)                    //删除当前商户的指定OPK
			storeGroup.POST("/defaultopk", pc.DefaultOPK)                //设置当前商户的默认OPK， 当OPK池为空，则需要用到此OPK
